...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{img/demo_curl.jpg}
\caption{Container Hailo berjalan di Raspberry Pi 5 (\texttt{docker ps}), endpoint
\texttt{/health} melaporkan \texttt{backend: hailo}, dan \texttt{/predict} mengembalikan
label kondisi lahan dari NPU.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{img/demo_poc.jpg}
\caption{PoC demo (\texttt{poc\_demo.py}) untuk tiga grid dan verifikasi throughput NPU
melalui \texttt{hailortcli run} (sekitar 15.231 FPS streaming).}
\end{figure}
